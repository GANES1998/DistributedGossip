...
-export([main/5, get_global_state/1, handle_message/9, listen_and_converge/7]).
...
handle_message(CurrentActor, MaxActors, {CurrentSum, CurrentWeight}, ListOfElements, GlobalMapping, Topology, SupervisorPid, {InputSum, InputWeight}, IsNewValue) ->

  if
    CurrentActor == 4 ->
      io:format("~p~n", [{CurrentSum, CurrentWeight}]);
    true -> io:format("")
  end,
...
%%             end,
  ...
  %% Will be used only if it is a new value
...
%%  NextPid2 ! {message_from_worker, {NewSum, NewWeight}},
  ...
  if
    IsNewValue ->
      listen_and_converge(CurrentActor, MaxActors, {NewSum, NewWeight}, util:update_fixed_list(ListOfElements, {NewSum / NewWeight, 1}, ConvergeLength),
      GlobalMapping, Topology, SupervisorPid);
    true -> listen_and_converge(CurrentActor, MaxActors, {CurrentSum, CurrentWeight}, ListOfElements,
      GlobalMapping, Topology, SupervisorPid)
  end.
    ...
    true ->
      receive
        {message_from_supervisor, {Element, Weight}} ->
          ...
          handle_message(CurrentActor, MaxActors, CurrentElement, CurrentElementList, GlobalMapping, Topology, SupervisorPid, {Element, Weight}, true);
        {message_from_worker, {Element, Weight}} ->
          handle_message(CurrentActor, MaxActors, CurrentElement, CurrentElementList, GlobalMapping, Topology, SupervisorPid, {Element, Weight}, true)
      after 1 ->
        handle_message(CurrentActor, MaxActors, CurrentElement, CurrentElementList, GlobalMapping, Topology, SupervisorPid, CurrentElement, false)
      end
